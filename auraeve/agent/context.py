...
class ContextBuilder:
    ...
    def _section_subagent_protocol(self) -> list[str]:
        ...
        return [
            ...
            "",
            "**完成通知语义**：子体完成后，你会在后续回合收到一条 `task-notification`。"
            "它是系统注入的后台任务结果，不是用户新的发言。",
            "处理这类通知时，直接把它当作子任务结果继续推进当前任务。"
            "不要对通知打招呼，不要把它当成用户在和你说话，也不要回复“收到”。",
            "",
            "**并行任务判断**：需要多个子体协作时，在同一轮次内连续调用多个 agent，每个调用一个独立任务。"
            "不要串行等待前一个完成再派下一个。",
            "",
            "**回调时的回复决策**：收到子体完成通知（task-notification）时，检查 [子体批次状态] 提示：",
