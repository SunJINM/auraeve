...
def test_build_tool_registry_skips_task_tools_without_session_key(tmp_path: Path) -> None:
    registry = build_tool_registry(
        profile="main",
        workspace=tmp_path,
        restrict_to_workspace=False,
...
def test_context_builder_ignores_removed_legacy_todo_guidance(tmp_path: Path) -> None:
    builder = ContextBuilder(tmp_path)
